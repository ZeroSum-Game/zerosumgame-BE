function initSocketHandlers({ io, prisma, auth, gameLogic, market }) {
  ...
  function computeMinigameTimeLimit(correctCount) {
    return Math.max(
      MINIGAME_MIN_SECONDS,
      MINIGAME_BASE_SECONDS - correctCount * 0.7,
    );
  }
  ...
  function buildMinigamePayload(state) {
    ...
    return {
      ...
      totalLeft: state.gameEndAt
        ? Math.max(0, Math.ceil((state.gameEndAt - Date.now()) / 1000))
        : 0,
  ...
  function computeTotalAssetForPlayer(player, marketRow) {
    ...
    const landTotal = Array.isArray(player.lands)
      ? player.lands.reduce(
          (sum, land) => sum + BigInt(land.purchasePrice || 0),
          0n,
        )
      : 0n;
    const stockTotal =
      BigInt(assets.samsung) * marketRow.priceSamsung +
    ...
    const timer = setInterval(() => {
      ...
      if (state.phase === "INTRO") {
        ...
        if (state.introLeft <= 0) {
          ...
          state.currentName =
            state.questionOrder[state.questionIndex] ||
            pickRandomName(state.questionOrder);
      ...
      if (state.phase === "GAME") {
        ...
        if (state.timeLeft <= 0) {
          ...
          state.players.forEach((p) => {
            if (!p.isDropped) p.isDropped = true;
          });
          endMinigame(roomId);
          return;
        }
  ...
  async function buildMinigamePlayers(roomId) {
    ...
    const players = activePlayers.length
      ? activePlayers
      : await gameLogic.getLatestPlayersByUser(prisma, roomId);
  ...
  async function syncLobbyPlayers(roomId) {
    ...
    if (!lobby.hostUserId && players.length > 0)
      lobby.hostUserId = players[0].userId;
    return lobby;
  }
  ...
  async function buildResumePayload(roomId) {
    ...
    const turnOrderPlayerIds = storedOrder
      ? storedOrder.filter((id) => playerIdSet.has(id))
      : playersPayload.map((p) => p.playerId);
    if (turnOrderPlayerIds.length)
    ...
    const playerIdToUser = new Map(
      playersPayload.map((p) => [p.playerId, p.userId]),
    );
    const turnOrder = turnOrderPlayerIds
      .map((id) => playerIdToUser.get(id))
      .filter((id) => id != null);
    const turnPlayerId = turnOrderPlayerIds.length
      ? await prisma.$transaction(async (tx) =>
          gameLogic.getTurnPlayerId(tx, roomId),
        )
      : null;
    let currentTurn = gameLogic.currentTurnUserByRoom.get(roomId) || null;
    if (!currentTurn && turnPlayerId) {
      const turnPlayer = await prisma.player.findUnique({
    ...
    if (currentTurn && !gameLogic.turnStateByRoom.get(roomId)) {
      gameLogic.turnStateByRoom.set(roomId, {
        userId: currentTurn,
        rolled: false,
        extraRoll: false,
      });
  ...
  async function ensureCurrentTurnUser(roomId) {
    ...
    const turnPlayerId = await prisma.$transaction(async (tx) =>
      gameLogic.getTurnPlayerId(tx, roomId),
    );
  ...
  async function buildStartPayload(roomId, turnOrderPlayerIds) {
    const players = await prisma.player.findMany({
      where: { id: { in: turnOrderPlayerIds } },
    });
    const users = await prisma.user.findMany({
      where: { id: { in: players.map((p) => p.userId) } },
      select: { id: true, nickname: true },
    ...
    const orderedPlayers = turnOrderPlayerIds
      .map((id) => byId.get(id))
      .filter(Boolean);
    ...
    const currentTurn =
      gameLogic.currentTurnUserByRoom.get(roomId) ||
      playersPayload[0]?.userId ||
      null;
    ...
    return {
      ...
      turnOrder,
      turnOrderPlayerIds,
      turnPlayerId: playersPayload[0]?.playerId || null,
    };
  }
  ...
  io.on("connection", (socket) => {
      ...
      try {
        ...
        if (!room) {
          room = await prisma.room.create({
            data: { id: resolvedRoomId, roomCode: "MAIN", status: "WAITING" },
          });
        }
        const lobbyState = getLobbyState(resolvedRoomId);
        const MAX_PLAYERS = 4;
        if (
          !lobbyState.players.has(sessionUserId) &&
          lobbyState.players.size >= MAX_PLAYERS
        ) {
          socket.emit("join_error", {
            message: `Room is full (${MAX_PLAYERS} max)`,
          });
          return;
        }
        ...
        if (room.status === "ENDED" && lobbyState.players.size === 0) {
          room = await prisma.room.update({
            where: { id: resolvedRoomId },
            data: { status: "WAITING", turnPlayerIdx: 0, currentTurn: 1 },
          });
        ...
        let player = await prisma.player.findFirst({
          where: { roomId: resolvedRoomId, userId: sessionUserId },
          orderBy: { id: "desc" },
          include: { assets: true },
        });
        if (!player) {
        ...
        if (room.status === "PLAYING") {
          ...
          if (!gameLogic.roomTurnOrder.get(resolvedRoomId)) {
            ...
            const fallbackOrder = activePlayers
              .slice()
              .sort((a, b) => a.id - b.id)
              .map((p) => p.id);
            if (fallbackOrder.length)
              gameLogic.roomTurnOrder.set(resolvedRoomId, fallbackOrder);
          }
        }
        ...
        lobby.players.set(sessionUserId, {
          userId: sessionUserId,
          nickname: user?.nickname || `Player${sessionUserId}`,
          playerId: player.id,
        ...
        const turnPlayerId = await prisma.$transaction(async (tx) =>
          gameLogic.getTurnPlayerId(tx, resolvedRoomId),
        );
        const marketData = await prisma.$transaction(async (tx) =>
          market.getOrCreateMarket(tx, resolvedRoomId),
        );
        ...
        socket.emit("join_success", {
          ...
          player,
          turnPlayerId,
          war: gameLogic.getWarPayload(),
          roomStatus: room.status,
          lobby: lobbyPayload,
          currentTurn:
            gameLogic.currentTurnUserByRoom.get(resolvedRoomId) || null,
        });
        socket.emit("market_update", {
          samsung: marketData.priceSamsung,
          tesla: marketData.priceTesla,
          lockheed: marketData.priceLockheed,
          gold: marketData.priceGold,
          bitcoin: marketData.priceBtc,
          prevSamsung: marketData.prevSamsung,
          prevTesla: marketData.prevTesla,
          prevLockheed: marketData.prevLockheed,
          prevGold: marketData.prevGold,
          prevBtc: marketData.prevBtc,
        });
    ...
    socket.on("set_ready", async ({ ready }) => {
      try {
        ...
        const player = await prisma.player.findFirst({
          where: { userId: info.userId, roomId: info.roomId },
          orderBy: { id: "desc" },
        });
        const isReady = !!ready;
        if (isReady && (!player || !player.character)) {
          socket.emit("ready_error", {
            message: "Select a character before ready",
          });
          return;
        }
        ...
        if (player) {
          ...
          if (player.character)
            io.to(info.roomId).emit("character_update", {
              playerId: player.id,
              userId: info.userId,
              character: player.character,
            });
      ...
      try {
        ...
        if (orderPickingByRoom.has(roomId)) {
          socket.emit("start_error", {
            message: "Order picking already started",
          });
          return;
        }
        ...
        const users = await prisma.user.findMany({
          where: { id: { in: activePlayers.map((p) => p.userId) } },
          select: { id: true, nickname: true },
        });
        const nicknameByUser = new Map(users.map((u) => [u.id, u.nickname]));
        ...
        if (activePlayers.some((p) => !p.character)) {
          socket.emit("start_error", {
            message: "All players must select a character",
          });
          return;
        }
    ...
    socket.on("minigame_answer", async (payload) => {
      try {
        ...
        if (answerChosung === state.currentChosung && isKnownName) {
          ...
          state.timeLeft = Math.ceil(state.timeLimit);
          state.questionIndex =
            (state.questionIndex + 1) % state.questionOrder.length;
    ...
    socket.on("pick_order_card", async (cardNumber) => {
      try {
        ...
        console.log(
          `[Order Pick] User ${userId} picked card ${cardId} => ${cardNumberValue}`,
        );
        ...
        io.to(info.roomId).emit("order_card_picked", {
          ...
          remainingCards: orderState.cardIds.filter(
            (c) => !Array.from(orderState.picks.values()).includes(c),
          ),
        });
        ...
        if (orderState.picks.size === orderState.players.length) {
          ...
          gameLogic.turnStateByRoom.set(info.roomId, {
            userId: currentTurn,
            rolled: false,
            extraRoll: false,
          });
          ...
          const nicknameByUser = new Map(
            lobbyInfo.players.map((p) => [p.userId, p.nickname]),
          );
          const allPlayers = await gameLogic.getLatestPlayersByUser(
            prisma,
            info.roomId,
          );
          ...
          const orderResults = sortedPlayers.map((p, idx) => ({
            ...
            playerId: p.playerId,
            cardNumber:
              orderState.cardValues.get(orderState.picks.get(p.userId)) || 0,
            turnOrder: idx + 1,
            nickname: p.nickname,
          }));
    ...
    socket.on("roll_dice", async () => {
      try {
        ...
        if (result.turnUserId)
          gameLogic.currentTurnUserByRoom.set(result.roomId, result.turnUserId);
        ...
        setTimeout(() => {
          try {
            io.to(result.roomId).emit("dice_rolled", {
              ...
              turnUserId: result.turnUserId,
              autoSellEvents: result.autoSellEvents,
            });
            io.to(result.roomId).emit("playerMove", {
              userId,
              playerId: result.player.id,
              character: result.player.character,
            ...
            if (result.market) {
              io.to(result.roomId).emit("market_update", {
            ...
            if (result.mapPriceUpdate) {
              io.to(result.roomId).emit("map_price_update", {
                prices: result.mapPriceUpdate,
              });
            }
            if (result.war) {
            ...
            if (result.tollOwnerId)
              gameLogic.emitAssetUpdate(result.tollOwnerId);
          } catch (err) {
            if (rollingUserByRoom.get(info.roomId) === userId) {
              rollingUserByRoom.delete(info.roomId);
              io.to(info.roomId).emit("dice_roll_cancelled", {
                userId,
                reason: "EMIT_FAILED",
              });
            }
            socket.emit("roll_error", {
              message: "Failed to broadcast dice result",
            });
            return;
          }
          if (rollingUserByRoom.get(info.roomId) === userId) {
    ...
    socket.on("draw_card", async (payload) => {
      try {
        ...
        io.to(info.roomId).emit("drawCard", {
          card,
          drawerUserId: info.userId,
      ...
      try {
        ...
        const result = await prisma.$transaction(async (tx) => {
          ...
          const players = await tx.player.findMany({
            where: { roomId },
            include: { assets: true, lands: true },
          });
          if (!players.length) throw new Error("Player not found");

          const drawer =
            players.find((p) => p.userId === info.userId) || players[0];
          const totals = players.map((p) => ({
            id: p.id,
            total: computeTotalAssetForPlayer(p, marketRow),
          }));
          const sorted = totals
            .slice()
            .sort((a, b) =>
          ...
          let landTollUpdate = null;
          let dividendOverride = null;
          let warBonusUpdate = null;

          const applyLandPriceChange = async (deltaRate) => {
            const landNodes = await tx.mapNode.findMany({
              where: { type: "LAND" },
            });
            const updates = {};
            for (const node of landNodes) {
              ...
              const nextPrice = Math.max(
                1,
                Math.round(current * (1 + Number(deltaRate || 0))),
              );
              await tx.mapNode.update({
                where: { nodeIdx: node.nodeIdx },
                data: { basePrice: nextPrice },
              });
          ...
          const applyLandTollChange = async (nodeIdxList, deltaRate) => {
            ...
            for (const nodeIdx of nodeIds) {
              ...
              const nextToll = Math.max(
                1,
                Math.round(current * (1 + Number(deltaRate || 0))),
              );
              await tx.mapNode.update({
                where: { nodeIdx },
                data: { baseToll: nextToll },
              });
              updates[nodeIdx] = nextToll;
            }
            return updates;
          };

          const applyLandTollByContinent = async (
            continent,
            deltaRate,
            extraIds = [],
          ) => {
            if (!continent) return {};
            const nodes = await tx.mapNode.findMany({
              where: { type: "LAND", continent },
            });
            const ids = nodes
              .map((n) => n.nodeIdx)
              .concat(Array.isArray(extraIds) ? extraIds : []);
            const uniqueIds = Array.from(new Set(ids));
            return applyLandTollChange(uniqueIds, deltaRate);
          };
          ...
          const applyMarketChange = async (symbol, deltaRate) => {
            ...
            const nextPrice = market.applyMarketDelta(
              currentPrice,
              Number(deltaRate || 0),
            );
          ...
          switch (card.id) {
            ...
            case 18: {
              warBonusUpdate = { playerId: drawer.id, bonus: card.effectValue ?? 0.05, turns: 2 };
              break;
            }
            case 12: {
            ...
            case 2: {
              const updates = await applyLandTollByContinent(
                card.continent,
                card.effectValue ?? 0.15,
              );
              landTollUpdate = { ...(landTollUpdate || {}), ...updates };
              break;
            }
            case 7: {
              landPriceUpdate = await applyLandPriceChange(
                card.effectValue ?? 0.1,
              );
              break;
            }
            case 8: {
              dividendOverride = { symbol: card.symbol || "SAMSUNG", rate: card.effectValue ?? 0.2, turns: 2 };
              break;
            }
            case 11: {
              const extraIds =
                card.extra && Array.isArray(card.extra.countryIds)
                  ? card.extra.countryIds
                  : [];
              const updates = await applyLandTollByContinent(
                card.continent,
                -Math.abs(card.effectValue ?? 0.15),
                extraIds,
              );
              landTollUpdate = { ...(landTollUpdate || {}), ...updates };
              break;
            ...
            case 15: {
              ...
              const updates = await applyLandTollChange(
                ids,
                -Math.abs(card.effectValue ?? 0.1),
              );
              landTollUpdate = { ...(landTollUpdate || {}), ...updates };
              break;
            }
        ...
        if (result.marketUpdated) {
          io.to(result.roomId).emit(
            "market_update",
            buildMarketPayload(result.updatedMarket),
          );
        }
        if (result.landPriceUpdate) {
          io.to(result.roomId).emit("map_price_update", {
            prices: result.landPriceUpdate,
          });
        }
        if (result.landTollUpdate) {
          io.to(result.roomId).emit("map_toll_update", {
            tolls: result.landTollUpdate,
          });
        }
        if (result.dividendOverride) {
          io.to(result.roomId).emit("dividend_override", result.dividendOverride);
        }
        if (result.warBonusUpdate) {
          gameLogic.setWarBonus(result.warBonusUpdate.playerId, result.warBonusUpdate.bonus, result.warBonusUpdate.turns);
          io.to(result.roomId).emit("war_bonus_update", result.warBonusUpdate);
        }
    ...
    socket.on("end_turn", async () => {
      try {
        ...
        await prisma.player.updateMany({
          where: { roomId: info.roomId, userId },
          data: { extraTurnUsed: false },
        });

        const lobbyPlayersSnapshot = Array.from(
          getLobbyState(info.roomId).players.values(),
        );
        const lobbyUserByPlayerId = new Map(
          lobbyPlayersSnapshot.map((p) => [p.playerId, p.userId]),
        );
        ...
        const turnResult = await prisma.$transaction(async (tx) => {
          ...
          const updateData = reachedEnd
            ? { status: "ENDED" }
            : {
                status: "PLAYING",
                turnPlayerIdx: nextIndex,
          ...
          return {
            ...
            reachedEnd,
            nextPlayerId,
            nextUserId: nextPlayer?.userId || fallbackUserId,
          };
        });

        let warEnded = false;
        if (gameLogic.warState.active) {
          gameLogic.warState.turnsLeft = Math.max(
            0,
        ...
        if (turnResult.reachedEnd) {
          const latestPlayers = await gameLogic.getLatestPlayersByUser(
            prisma,
            info.roomId,
          );
          gameLogic.tickWarBonuses(latestPlayers.map((p) => p.id));
          ...
          // 우승자 결정
          const winnerId = rankings.length > 0 ? rankings[0].playerId : null;

          // 게임 종료 데이터
          const gameEndData = {
            rankings,
            winnerId,
            maxTurn: turnResult.room.maxTurn,
            players: playersPayload,
            reason: "최대 라운드 도달",
            endedAtRound:
              turnResult.room.currentTurn || turnResult.room.maxTurn,
          };

          console.log(
            `[GAME_END] Broadcasting to room ${info.roomId}, winner: ${winnerId}, players: ${latestPlayers.length}`,
          );

          // 방 전체에 브로드캐스트
          io.to(info.roomId).emit("game_end", gameEndData);

          // 각 플레이어의 소켓에 개별적으로도 전송 (이중 보장)
          try {
            for (const player of latestPlayers) {
              if (player.socketId && io.sockets.sockets.has(player.socketId)) {
                const playerSocket = io.sockets.sockets.get(player.socketId);
                playerSocket.emit("game_end", gameEndData);
                console.log(
                  `[GAME_END] Sent to player ${player.userId} (socket: ${player.socketId})`,
                );
              } else {
                console.log(
                  `[GAME_END] Warning: Player ${player.userId} has no active socket`,
                );
              }
            }
          } catch (err) {
            console.error(
              "[GAME_END] Error sending individual socket events:",
              err,
            );
          }
        ...
        try {
          ...
          gameLogic.tickWarBonuses(latestPlayers.map((p) => p.id));
          await Promise.all(
            latestPlayers.map((p) => gameLogic.emitAssetUpdate(p.id)),
          );
        } catch (err) {}

        if (turnResult.nextUserId) {
          gameLogic.currentTurnUserByRoom.set(
            info.roomId,
            turnResult.nextUserId,
          );
          gameLogic.turnStateByRoom.set(info.roomId, {
            userId: turnResult.nextUserId,
            rolled: false,
            extraRoll: false,
          });
        } else {
        ...
        io.to(info.roomId).emit("turn_update", {
          turnPlayerId: turnResult.nextPlayerId,
          currentTurn: turnResult.nextUserId,
          war: warPayload,
        });
      } catch (err) {
        socket.emit("turn_error", {
          message: err?.message || "Failed to end turn",
        });
      }
    });
    ...
    socket.on("start_war_fight", async ({ opponentUserId }) => {
      const info = socketUserMap.get(socket.id);
      if (!info)
        return socket.emit("war_error", { message: "Not authenticated" });

      try {
        const result = await prisma.$transaction(async (tx) => {
          const attacker = await tx.player.findFirst({
            where: { userId: info.userId, roomId: info.roomId },
            include: { assets: true },
          });
          const defender = await tx.player.findFirst({
            where: { userId: opponentUserId, roomId: info.roomId },
            include: { assets: true },
          });
          ...
          // 자산 가치 계산
          const attackerAsset = gameLogic.calcWarAssetValue(
            attacker.assets,
            market,
            attacker.cash,
          );
          const defenderAsset = gameLogic.calcWarAssetValue(
            defender.assets,
            market,
            defender.cash,
          );
          ...
          // 승률 계산
          let winRate = gameLogic.calcWarWinRate({
            myAsset: attackerAsset,
            oppAsset: defenderAsset,
            character: attacker.character,
          });
          const defenderBonus = gameLogic.getWarBonus(defender.id);
          winRate = gameLogic.applyWarBonus(winRate, defenderBonus);
        ...
        socket.emit("war_error", {
          message: err?.message || "전쟁 처리에 실패했습니다.",
        });
      }
    });
    ...
    socket.on("select_war_spoils", async ({ winnerId, loserId, landId }) => {
      ...
      try {
        const result = await prisma.$transaction(async (tx) => {
          const winner = await tx.player.findUnique({
            where: { id: winnerId },
          });
          ...
          if (landId !== null) {
            ...
            const mapNode = await tx.mapNode.findUnique({
              where: { nodeIdx: landId },
            });
            ...
            const actualPenalty =
              loser.cash < cashPenalty ? loser.cash : cashPenalty;
        ...
        socket.emit("war_error", {
          message: err?.message || "전리품 선택에 실패했습니다.",
        });
      }
    });
    ...
    socket.on("disconnect", async () => {
      ...
      if (minigameState && minigameState.phase === "GAME") {
        const player = minigameState.players.find(
          (p) => p.userId === info.userId,
        );
        if (player && !player.isDropped) {
          player.isDropped = true;
          const aliveCount = minigameState.players.filter(
            (p) => !p.isDropped,
          ).length;
          if (aliveCount <= 1) {
